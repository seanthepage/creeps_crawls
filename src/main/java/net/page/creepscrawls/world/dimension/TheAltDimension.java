...
public class TheAltDimension extends CreepsCrawlsModElements.ModElement {
	@ObjectHolder("creeps_crawls:the_alt_portal")
	public static final CustomPortalBlock portal = null;
	public TheAltDimension(CreepsCrawlsModElements instance) {
		super(instance, 4);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(Blocks.COAL_BLOCK.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("creeps_crawls:the_alt"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_alt_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("the_alt_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("the_alt_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new TheAltItem().setRegistryName("the_alt"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote
				...
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("creeps_crawls:the_alt"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("creeps_crawls:the_alt")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.COAL_BLOCK.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? Blocks.COAL_BLOCK.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
